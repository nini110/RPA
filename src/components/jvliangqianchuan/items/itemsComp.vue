<template>
	<div class="itemsComp">
		<div class="centers">
			<el-button type="primary" @click="newStrategyFn()" size="medium">新建项目</el-button>
			<div class="tables">
				<el-table :data="tableDataList" style="width: 100%" @cell-click="cellClick">
					<el-table-column align="center" label="序号" width="120" type="index">
					</el-table-column>
					<el-table-column prop="project_name" label="项目名称" width="180" align="center" header-align="center">
					</el-table-column>
					<el-table-column prop="shop_name" label="抖店/代理商" width="180" align="center" header-align="center">
					</el-table-column>
					<el-table-column prop="quanchuan_name" label="千川账号" width="180" align="center" header-align="center">
					</el-table-column>
					<el-table-column prop="douyin_name" label="抖音账号" width="180" align="center" header-align="center">
					</el-table-column>
					<el-table-column prop="address3" label="投放时间" width="180" align="center" header-align="center">
						<template slot-scope="scope">
							<div>{{ scope.row.cast_start_time }}</div>
							<div>{{ scope.row.cast_end_time }}</div>
						</template>
					</el-table-column>
					<el-table-column prop="interval" label="数据获取间隔" width="120" align="center" header-align="center">
						<template slot-scope='scope'>
							<div>{{ scope.row.interval/60 }}分钟</div>
						</template>
					</el-table-column>
					<el-table-column prop="address6" label="绑定策略数" width="100" align="center" header-align="center">
						<template slot-scope='scope'>
							<div>1</div>
						</template>
					</el-table-column>
					<el-table-column prop="address7" label="创建时间" width="120" align="center" header-align="center">
						<template slot-scope='scope'>
							<div>2021-08-27</div>
						</template>
					</el-table-column>
					<el-table-column prop="founder" label="创建者" width="80" align="center" header-align="center">
					</el-table-column>
					<el-table-column label="操作" width="180" align="center" header-align="center"
						...
						<template slot="header">
							...
							<el-tooltip effect="dark" content="" placement="top">
								<div slot="content">项目开始前10分钟无法再编辑和删除项目<br />开始前5分钟可以进入实时看板</div>
						...
						<template slot-scope="scope">
							...
							<el-button type="text" @click="routerLink(scope.row.id,scope.row.project_name)">实时看板</el-button>
						</template>
					</el-table-column>
				</el-table>
			</div>
			...
			<div class="dialog">
				<el-dialog title="新建/编辑" :visible.sync="dialogVisible" width="800px" :before-close="handleClose">
					<el-form ref="form" :model="form" label-width="120px">
						<el-form-item label="项目名称:" prop="name">
							<el-input v-model="form.name" placeholder="请输入项目名称" size="mini" class="w240"></el-input>
						</el-form-item>
						...
						<el-form-item label="授权账号:">
							<el-select v-model="form.authorization" placeholder="请选择授权账号" size="mini" class="w240"
								@change="handleAuthorization">
								<el-option v-show="form.radio === '1'" v-for="(item,index) in authorizationOption"
									:label="item.advertiser_name" :value="[item.advertiser_id,item.advertiser_name]"></el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="千川账号:">
							<el-select v-model="form.ThousandsOfSichuan" placeholder="请选择千川账号" size="mini" class="w240"
								@change="handleThousandsOfSichuan">
								<el-option v-for="(item,index) in ThousandsOfSichuanOptions" :label="item.name"
									:value="[item.id,item.name]"></el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="抖音账号:">
							<el-select v-model="form.trill" placeholder="请选择抖音账号" size="mini" class="w240" @change="handleTrill">
								<el-option v-for="(item,index) in trillOptions.aweme_id_list" :label="item.aweme_name"
									:value="[item.aweme_id,item.aweme_name]"></el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="投放时间:" size="mini">
							<div class="block">
								<el-date-picker v-model="form.timeData" type="datetimerange" range-separator="---" @change="times"
						...
						<el-tabs type="border-card">
							<el-tab-pane label="出价调整">
								<div class="cjtabbles">
									<el-select v-model="form.cjstrategy" placeholder="请选择千川账号" size="mini" class="w240">
										<el-option v-for="(item,index) in cjtableData" :label="item.name"
							...
							<el-tab-pane label="预算调整">
								<div class="ystabbles">
									<el-select v-model="form.ysstrategy" placeholder="请选择千川账号" size="mini" class="w240">
										<el-option v-for="(item,index) in ystableData" :label="item.name"
											:value="item.id"></el-option>
									</el-select>
								</div>
							</el-tab-pane>
							<el-tab-pane label="计划复制">
								<div class="jhtabbles">
									<el-select v-model="form.jhstrategy" placeholder="请选择千川账号" size="mini" class="w240">
										<el-option v-for="(item,index) in jhtableData" :label="item.name"
											:value="item.id"></el-option>
									</el-select>
								</div>
							</el-tab-pane>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 编辑更新项目
			StrategiesUpdate(){
				if(this.form.cjstrategy || this.form.jhstrategy || this.form.ysstrategy){
					let data = {
					...
					StrategiesUpdate(data).then(res=>{
						if(res.code === 200){
			...
			projectEdit(ids){
				...
				projectEdit(id).then(res=>{
					...
					let start3 = start1[0]+start1[1]+start2[0]+start2[1]+start2[2];
					...
					console.log(end5)
					this.form.timeData = [new Date(parseInt(start5[0]+start5[1]),start5[2]-1,start5[3],start5[4],start5[5],start5[6]), new Date(parseInt(end5[0]+end5[1]),end5[2]-1,end5[3],end5[4],end5[5],end5[6])]
					// timeData: [new Date(2000, 10, 10, 10, 10), new Date(2000, 10, 11, 10, 10)],
			...
			Strategies(id){
				...
				Strategies(params).then(res=>{
					console.log(res);
					this.form.cjstrategy = res.data.bid_strategy_data.id;
					this.form.ysstrategy = res.data.budget_strategy_data.id;
					this.form.jhstrategy = res.data.plan_strategy_data.id;
				}).catch(err=>{
					console.log(err)
				})
			},
			...
			projectList() {
				...
				projectList(params).then(res => {
					this.tableDataList = res.results;
			...
			strategyList() {
				...
				strategyList(params).then(res => {
					this.cjtableData = res.data.data;
				}).catch(err => {
					console.log(err)
				})
			},
			...
			budgetStrategyList(){
				...
				budgetStrategyList(params).then(res => {
					this.ystableData = res.data.data;
				}).catch(err => {
					console.log(err)
				})
			},
			...
			planStrategyList(){
				...
				planStrategyList(params).then(res => {
					this.jhtableData = res.data.data;
				}).catch(err => {
					console.log(err)
				})
			},
			...
			// 抖音账号下拉
			getTrill(params) {
				getTrill(params).then(res => {
					this.trillOptions = res.data;
				}).catch(err => {
					console.log(err)
				})
			},
			...
			getThousandsOfSichuan(params) {
				getThousandsOfSichuan(params).then(res => {
					
					this.ThousandsOfSichuanOptions = res.data;
				}).catch(err => {
					console.log(err)
				})
			},
