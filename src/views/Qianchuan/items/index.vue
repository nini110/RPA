		...
		<div class="centers">
			...
			<div class="tabbles itemtable" ref="tabbles">
				<el-table :data="tableDataList" height="0" tooltip-effect="dark" @cell-click="cellClick"
					...
					<el-table-column prop="interval" label="数据获取间隔" min-width="200">
						<template slot-scope="scope">
							<div>{{ scope.row.interval / 60 }}分钟</div>
						</template>
					</el-table-column>
					<el-table-column prop="address6" label="绑定策略数" min-width="200">
						<template slot-scope="scope">
							<div>1</div>
						</template>
					</el-table-column>
					<el-table-column prop="address7" label="创建时间" min-width="200">
						<template slot-scope="scope">
							<div>2021-08-27</div>
						</template>
					</el-table-column>
					...
					<el-table-column label="操作" width="280" fixed="right">
						...
						<template slot-scope="scope">
							...
							<el-button type="text" class="el-icon-time "
								@click="routerLink(scope.row.id, scope.row.project_name)">实时看板</el-button>
						</template>
					</el-table-column>
				</el-table>
			</div>
			...
			<div class="dialog">
				<el-dialog title="新建/编辑" :visible.sync="dialogVisible"
					custom-class="dialogEdit dialogStrategy dialogItem" :close-on-click-modal="false">
					<el-form ref="form" :model="form">
						<el-form-item label="项目名称:" prop="name">
							<el-input v-model="form.name" placeholder="请输入项目名称" size="medium" clearable></el-input>
						</el-form-item>
						...
						<el-form-item label="授权账号:">
							<el-select v-model="form.authorization" placeholder="请选择授权账号" size="medium" clearable
								@change="handleAuthorization">
								<el-option v-show="form.radio === '1'" v-for="(item, index) in authorizationOption"
									:key="index" :label="item.advertiser_name"
									:value="[item.advertiser_id, item.advertiser_name]"></el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="千川账号:">
							<el-select v-model="form.ThousandsOfSichuan" placeholder="请选择千川账号" size="medium" clearable
								@change="handleThousandsOfSichuan">
								<el-option v-for="(item, index) in ThousandsOfSichuanOptions" :key="index"
									:label="item.name" :value="[item.id, item.name]"></el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="抖音账号:">
							<el-select v-model="form.trill" placeholder="请选择抖音账号" size="medium" clearable
								@change="handleTrill">
								<el-option v-for="(item, index) in trillOptions.aweme_id_list" :key="index"
									:label="item.aweme_name" :value="[item.aweme_id, item.aweme_name]"></el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="投放时间:" size="medium">
							<el-date-picker v-model="form.timeData" type="datetimerange" range-separator="---"
								@change="times" value-format="yyyy-MM-dd HH:mm:ss" format="yyyy-MM-dd HH:mm:ss"
						...
						<el-form-item label="绑定策略:">
							<el-tabs type="border-card">
								<el-tab-pane label="出价调整">
									<div class="cjtabbles">
										<el-select v-model="form.cjstrategy" placeholder="请选择千川账号" size="medium">
											<el-option v-for="(item, index) in cjtableData" :key="index"
								...
								<el-tab-pane label="预算调整">
									<div class="ystabbles">
										<el-select v-model="form.ysstrategy" placeholder="请选择千川账号" size="medium">
											<el-option v-for="(item, index) in ystableData" :key="index"
												:label="item.name" :value="item.id"></el-option>
										</el-select>
									</div>
								</el-tab-pane>
								<el-tab-pane label="计划复制">
									<div class="jhtabbles">
										<el-select v-model="form.jhstrategy" placeholder="请选择千川账号" size="medium">
											<el-option v-for="(item, index) in jhtableData" :key="index"
												:label="item.name" :value="item.id"></el-option>
										</el-select>
									</div>
								</el-tab-pane>
					...
					<span slot="footer" class="dialog-footer">
						...
						<el-button class="btnnormal" type="primary" @click="isOk()" size="medium">确 定</el-button>
					</span>
				</el-dialog>
			</div>
		</div>
