...
<template>
	...
	<div class="PlanToCopy strategyNormal">
		<div class="centers">
			...
			<!-- 复制计划策略弹窗 -->
			<div class="dialog">
				<el-dialog title="复制计划策略" :visible.sync="dialogVisible" custom-class="dialogEdit dialogStrategy"
					...
					<el-form ref="form" :model="form" :rules="rules">
						...
						<el-form-item label="取数次数:" prop="triesLimit" class="oneIpt">
							<el-input-number v-model="form.triesLimit" @change="handleChange" :min="1" :max="50"
								size="medium" label="描述文字" clearable></el-input-number>次
						</el-form-item>
						<el-form-item label="判断依据:" prop="judge" class="twoIpt">
							<el-select v-model="form.judge" placeholder="请选择" size="medium" clearable>
								<el-option v-for="item in judgeOptions" :key="item.value" :label="item.label"
									:value="item.value">
								</el-option>
							</el-select>
							<!-- ≥ -->
							<el-input-number v-model="form.judgeNum" @change="handleChange" :min="1" :max="50"
