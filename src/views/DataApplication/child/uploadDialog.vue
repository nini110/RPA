<template>
	<!-- 上传竞标 -->
	<el-dialog title="上传竞标" :visible.sync="show" @close="closeDialog" width="35%" max-height="800px"
		custom-class="dialogJb" :close-on-click-modal="false">
		<el-form ref="form" :model="form" class="formObj" :rules="rules">
			<el-row>
				<el-col>
					<el-form-item label="选择竞标:" prop="bidId">
						<el-select v-model="form.bidId" filterable remote reserve-keyword placeholder="请输入标名或者项目编号"
							:remote-method="searchEvent" :loading="loading" :popper-append-to-body="false" clearable
							@change="jbChangeEvent">
							<el-option v-for="item in bidOptions" :key="item.value"
								:label="`编号:${item.pro_num}——标名：${item.pro_name}`" :value="item.pro_num">
							</el-option>
						</el-select>
					</el-form-item>
				</el-col>
				...
				<el-col>
					<el-form-item label="添加人员:" prop="cSubcategoryNo">
						<el-input class="inline-input" v-model.trim="cSubcategoryNo" placeholder="请输入上传人员姓名或关键字"
							@input="searchPeople" size="medium" clearable></el-input>
					</el-form-item>
				</el-col>
				<el-col>
					<el-collapse-transition>
						<el-form-item v-if="peopleOptions.length > 0" label="" class="tagBox">
							<el-scrollbar style="height: 100%">
								<el-tag v-for="tag in peopleOptions" :disable-transitions="false" :key="tag.userid"
									type="" @close="handleClose(tag)" @click="addPerson(tag)">{{ tag.name }}</el-tag>
							</el-scrollbar>
						</el-form-item>
					</el-collapse-transition>
				</el-col>
		...
		<div class="send">
			<div class="namelist">
				<el-scrollbar style="height: 100%">
					<ul style="list-style: none">
						<li v-for="(i, index) in peoplelist" :key="index" :index="index" style="margin-bottom: 5px"
			...
			<div class="上传" style="flex: 1">
				<Upload class="tes" txt="请上传xlsx文件，通过搜索添加上传人员，双击上传人员可删除" :progressPercent="form.progressPercent"
					@getFile="getFileEvent"></Upload>
			</div>
		</div>
		<span slot="footer" class="dialog-footer">
			<a class="btnnormal_down" href="http://tool.afocus.com.cn/file_download/预算监控模板.xlsx" download="预算监控模板.xlsx">
				<div class="btnSize">下载模板</div>
			</a>
			<el-button v-waves class="marginL" type="primary" @click="uploadFile">立即上传</el-button>
		</span>
	</el-dialog>
</template>
<script>
