						...
						<el-row>
							<el-col :span="12">
								<el-form-item label="项目:" prop="project_name">
									<el-select v-model="form.project_name" placeholder="请选择项目" size="medium" clearable
										@change="selectChang">
										<el-option v-for="(item, index) in itemOptions" :key="index"
											:label="item.project_name" :value="item.project_name">
										</el-option>
									</el-select>
								</el-form-item>
							</el-col>
							<el-col :span="12">
								<el-form-item class="hasdate" label="数据日期:" prop="date">
									...
									<el-date-picker v-model="form.date" type="date" size="medium" @change="dateChange"
										...
										:disabled="dateDis" :picker-options="pickerOptionsStart">
									</el-date-picker>
								</el-form-item>
							</el-col>
							<el-col :span="24">
					...
					<div class="tableTab">
						...
						<el-table class="tableBox" ref="multipleTable" :data="tableData" tooltip-effect="dark"
							...
							<el-table-column prop="id" label="操作" width="220">
								<template slot-scope="scope">
									<el-button v-waves :disabled="scope.row.status !== 1"
										class="btn btn_info el-icon-download" @click="downEvent(scope.row)">
									</el-button>
									<el-button v-waves :disabled="scope.row.status === 0"
										class="btn btn_delete el-icon-delete" @click="deleteEvent(scope.row)">
									</el-button>
								</template>
							</el-table-column>
						</el-table>
				...
				vm.$refs.form.validate(valid => {
					if (valid) {
						if (vm.checkedItem.file) {
							vm.errorUploadInfo = vm.fileList && vm.fileList.length !== 0 ? '' : '请上传文件'
